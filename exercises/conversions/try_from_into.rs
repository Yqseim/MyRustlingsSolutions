...
fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
    ...
    if tuple.1 < RGB_MIN || tuple.1 > RGB_MAX {
        return Err("Invalid G Value".to_string());
    }

    if tuple.2 < RGB_MIN || tuple.2 > RGB_MAX {
        return Err("Invalid B Value".to_string());
    }
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        for item in &arr {
            if item < &RGB_MIN || item > &RGB_MAX {
               return Err("One item is invalid".to_string());
            }
        }
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        for s in slice.iter() {
            if s < &RGB_MIN || s > &RGB_MAX {
               return Err("One slice item is invalid".to_string());
            }
        }
